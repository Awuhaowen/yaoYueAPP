<template>
	<view class="segmented-control" :class="styleType" :style="wrapStyle">
		<view
			...
			:class="styleType"
			:key="index"
			:style="index === currentIndex ? activeStyle : itemStyle"
			@click="onClick(index)"
		>
			{{ item }}
		</view>
		...
		itemStyle() {
			...
			switch (this.styleType) {
				case 'text':
					...
					break;
				default:
					styleString = `color:${this.activeColor};border-color:${
						this.activeColor
					};`;
					break;
			}
		...
		activeStyle() {
			...
			switch (this.styleType) {
				case 'text':
					...
					break;
				default:
					styleString = `color:#fff;border-color:${
						this.activeColor
...
.segmented-control-item.button {
	border-left: 1upx solid #ededed !important;
}
